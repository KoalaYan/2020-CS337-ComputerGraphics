...
class SoccerBallTracker:
    ...
    def detect_yolo(self, img):
        ...
        min_wh = min(frame_width, frame_height)
        max_wh = int(max_wh/32) * 32
        min_wh = int(min_wh/32+1) * 32
        # Create a 4D blob from a frame.
        blob = cv2.dnn.blobFromImage(img, 1 / 255, (self.inpWidth, self.inpHeight), [0, 0, 0], 1, crop=False)
        print(max_wh)
        print(min_wh)
        # blob = cv2.dnn.blobFromImage(img, 1 / 255, (max_wh, min_wh), [0, 0, 0], 1, crop=False)
        ...
        for out in outs:
            for detection in out:
                if detection[4] > self.objectnessThreshold:
                    ...
                    #     confidence_p = scores[classId]
                    #     if confidence_p > self.confThreshold:
                    #         center_p_x = int(detection[0] * frame_width)
                    #         center_p_y = int(detection[1] * frame_height)
                    #         width_p = int(detection[2] * frame_width)
                    #         height_p = int(detection[3] * frame_height)
                    #         left_p = int(center_p_x - width_p / 2)
                    #         top_p = int(center_p_y - height_p / 2)
                    #         classIds.append(classId)
                    #         confidences.append(float(confidence_p))
                    #         boxes.append([left_p, top_p, width_p, height_p])
    ...
    def detect_and_track(self, img):
        ...
        is_detecting = False
        if (self.frameNumber % 5 == 0) or (not self.tracking_ok):
